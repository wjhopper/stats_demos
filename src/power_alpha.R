power_alpha <- function(meanh0 = 100, effect =5, N = 25, sdh0 = 15, interval = 2.5, frames = 10) {
  ...
  ## use a loop to create images one by one over some nice alpha levels
  for (alpha in seq(.2,.01, by=-.2/frames)) {
    ...
    #     power = round( pnorm(meanh0 - cutoff*se, meanh0 + effect, sdh0/sqrt(N)) + 1 - pnorm(meanh0 + cutoff*sdh0/sqrt(N), meanh0 + effect, sdh0/sqrt(N)),3)
